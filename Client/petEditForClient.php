<?php
    ...
    $pet = soloPetView($link, $idPet);

    if ($_POST['drone'] ==='male'){
        $sex = 'Самець';
    } 
    if ($_POST['drone'] === 'female'){
        $sex = 'Самка';
    }

    if($_GET['action'] === 'edit'){
        updatePet($link, $idPet, $_POST['alias'], $_POST['kind'], $_POST['breed'], $_POST['age'], $sex);
        $new_url = 'http://diploma/Client/personal.php';
        header('Location: '.$new_url);
    }
    
    if ($_GET['action'] === 'delete'){
        petDelete($link, $idPet);
        $new_url = 'http://diploma/Client/personal.php';
        header('Location: '.$new_url);
    }
?>
...
<meta charset="utf-8">
    ...
    <title>Редагування запису про тварину</title>
</head>
<body>
    ...
    <nav class = "navigation">
        <ul>
            ...
            <?php if ($loginUser == "user"){?>
                ...
                <li><a href = "personal.php">Особистий кабінет</a></li>
            ...
            <?php } else if ($loginUser == "doctor"){?>
                ...
                <li><a href = "../Doctor/petEdit.php">Редагувати тварину</a></li>
                <li><a href = "../dateTable.php">Розклад</a></li>
                <li><a href = "../Doctor/newRecord.php">Запис на прийом</a></li>
                <li><a href ="../Reg/logout.php">Вийти</a></li>
            ...
            <div class="dropdown">
                <button class="dropbtn">Клієнти</button>
                    ...
                    <a href="clientView.php">Перегляд</a>
                    <a href="clientEdit.php">Редагувати</a>
                    <a href="clientAdd.php">Додати</a>
                </div>
            </div>
            <div class="dropdown">
    ...
    <form method="post" enctype="multipart/form-data" action = "petEditForClient.php?id=<?=$_GET['id']?>&&action=edit">
        <br>
        <label>Кличка тварини </label>
            <input type="text" name="alias" value="<?=$pet['alias']?>" required>
        <br>
        <label>Вид
            <input type="text" name="kind" value="<?=$pet['kind']?>" required>
        <br>
        <label>Порода </label>
