<?php
    ...
    if ($loginUser == 'doctor' or $loginUser == 'admin' or $loginUser == 'user'){
        require("Other/database.php");
        require("Other/function.php");
        

        if ($_GET['action'] === 'delete'){
            receptionDelete($link, $_GET['id']);
            header("Location: dateTable.php");
        }

        if($_GET['startDate'] != ''){
            $startDate = $_GET['startDate'];
            $endDate = $_GET['endDate'];
            $receptions = receptionPeriod($link, $startDate, $endDate);
        } else $receptions = receptionsViews($link);
?>
...
<meta charset="utf-8">
    ...
    <title>Розклад за весь час</title>
</head>
<body>
    ...
    <nav class = "navigation">
        <ul>
            ...
            <?php if ($loginUser == "user"){?>
                ...
                <li><a href = "Doctor/diagnosisView.php">Список діагнозів</a></li>
                <li><a href = "Doctor/doctorView.php">Список лікарів</a></li>
                <li><a href = "Doctor/newRecord.php">Запис на прийом</a></li>
                <li><a href = "Client/personal.php">Особистий кабінет</a></li>
                <li><a href ="Reg/logout.php">Вийти</a></li>
            <?php } else if ($loginUser == "doctor"){?>
            <li><a href = "Doctor/diagnosisView.php">Список діагнозів</a></li>
            <li><a href = "Doctor/petView.php">Список тварин</a></li>
            <li><a href = "Doctor/petAdd.php">Додати тварину</a></li>
            <li><a href = "Doctor/Edit.php">Редагувати тварину</a></li>
            <li><a href = "dateTable.php">Розклад</a></li>
            <li><a href = "Doctor/newRecord.php">Запис на прийом</a></li>
            <li><a href ="Reg/logout.php">Вийти</a></li>
            ...
            <div class="dropdown">
                <button class="dropbtn">Тварини</button>
                    <div class="dropdown-content">
                    <a href="Doctor/petView.php">Перегляд</a>
                    <a href="Doctor/petEdit.php">Редагувати</a>
                    <a href="Doctor/petAdd.php">Додати</a>
                </div>
            </div>
            <div class="dropdown">
                <button class="dropbtn">Лікарі</button>
                    <div class="dropdown-content">
                    <a href="Doctor/doctorView.php">Перегляд</a>
                    <a href="Doctor/doctorEdit.php">Редагувати</a>
                    <a href="Doctor/doctorAdd.php">Додати</a>
                </div>
            </div>
            <div class="dropdown">
                <button class="dropbtn">Аналіз</button>
                    <div class="dropdown-content">
                    <a href="Doctor/servMonths.php">Прибуток по місяцям</a>
                    <a href="Doctor/docAnalysis.php">Лікарів</a>
                    <a href="Doctor/petAnalysis.php">Тварин</a>
                </div>
            </div>
            <li><a href = "Doctor/diagnosisView.php">Список діагнозів</a></li>
            <li><a href = "dateTable.php">Розклад</a></li>
    ...
    <div class = "mainText">
    <h2><p>Розклад за певний період</p></h2>
    <form method = 'GET'  action = "dateTable.php">
        <label>Початок періода </label>
        <input type="date" name="startDate" required>
        <label>Кінець періода </label>
        <input type="date" name="endDate" required>
        <input type="submit" value="Переглянути">
    </form>

    <h2><p>Розклад за <?php if($_GET['startDate'] != '') {?> період з <?=$startDate?> до <?=$endDate?> <?php } else{?> весь час <?php };?></p></h2>
    <div class="tablePet">
    <table>
        <tr>
            <th>Ім'я лікаря</th>
        ...
        <?php foreach($receptions as $reception): ?>
 		<tr>
 			<td><?php echo (doctorName($link, $reception['doctor']))?></td>
 			<td><?php echo (petAlias($link, $reception['pet']))?></td>
            <td><?php echo (diagnosName($link, $reception['diagnos']))?></td>
            <td><?=$reception['date']?></td>
            <td><?=$reception['time']?></td>
            <?php if( $loginUser == 'admin'){?>
            <td><a href = "dateTable.php?id=<?=$reception['id']?>&&action=delete">Видалити</a></td>
            <?php }?>
 		</tr>
        <?php endforeach ?>
    </table>
