<?php
    ...
    if ($loginUser == 'doctor' or $loginUser == 'admin'){
        ...
        if ($_FILES && $_FILES["image"]["error"]== UPLOAD_ERR_OK){
            $_FILES["image"]["name"] = idForImagePet($link) . ".jpg";
            $upload_image = $_FILES["image"]['name'];
            $folder = "../Client/imgPet/";
            ...
            if (addPet($link, $_POST['nameOwner'], $_POST['alias'], $_POST['kind'], $_POST['breed'], $_POST['age'], $sex, $upload_image, $folder) === true){
                move_uploaded_file($_FILES["image"]["tmp_name"],"$folder".$_FILES["image"]["name"]);
                $new_url = 'http://diploma/index.php';
...
<body>
    ...
    <form method="post" enctype="multipart/form-data" action = "petAdd.php">
        <br>
        <label>Код власника </label>
