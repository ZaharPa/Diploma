<?php
    ...
    if ($loginUser == 'doctor' or $loginUser == 'admin' or $loginUser == 'user'){
        ...
        $doctors = doctorView($link);
        $pets = petView($link);
        $diagnosis = diagnosisView($link);
        ...
        $time = $_POST['time'];
        if ($doctor != ''){
            if(checkReception($link, $doctor, $pet, $date, $time) === true){
                recordReception($link, $doctor, $pet, $diagnos, $date, $time);
                header("Location: ../index.php");}
            else if (checkReception($link, $doctor, $pet, $date, $time) === false){
                $error = "Цей час є зайнятим, будь ласка змініть час";
            }
        }
...
<body>
    ...
    <h2>Новий запис на прийом до лікаря</h2>
    <form method="post" enctype="multipart/form-data" action = "newRecord.php">
        <br>
        <label>Лікар </label>
        <select name="doctor" required>
            <?php foreach($doctors as $doc): ?>
                <option selected value = <?=$doc['id']?>><?=$doc['ПІБ']?></option>
                <?php endforeach;?>
        </select>
        <br>
        <label>Тварина </label>
        <select name="pet" required>
            <?php foreach($pets as $pet): ?>
                <option  value = <?=$pet['id']?>><?=$pet['alias']?>, <?= petOwnerView($link, $pet['nameOwner'])?></option>
            <?php endforeach;?>
        </select>
        <br>
        <label>Діагноз, якщо відомо
        <select name="diagnos" >
        ...
        <br>
        <label for="time">Виберість час для прийому:</label>

        <input type="time" id="time" name="time" min="09:00" max="18:00" required>

        <small>Лікарня працює з 9:00 до 18:00</small>
        <br>
