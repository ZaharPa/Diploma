<?php
    ...
    if ($loginUser == 'admin'){
        ...
        $doc = soloDocView($link, $_GET['id']);

        $id = $_GET['id'];
        if($_GET['action'] === 'edit'){
            updateDoctor($link, $id, $_POST['login'], $_POST['password'], $_POST['name'], $_POST['special'], $_POST['cab'], $_POST['exp']);
            $new_url = 'http://diploma/Doctor/doctorEdit.php';
...
<meta charset="utf-8">
    ...
    <title>Редагування запису про лікаря</title>
</head>
<body>
    ...
    <form method="post" enctype="multipart/form-data" action = "doctorEditForm.php?id=<?=$_GET['id']?>&&action=edit">
        <br>
        <label>Логін</label>
            <input type="text" name="login" value="<?=$doc['login']?>" required>
        <br>
        <label>Пароль</label>
            <input type="password" name="password" value="<?=$doc['password']?>" required>
        <br>
        <label>ПІБ
            <input type="text" name="name" value="<?=$doc['ПІБ']?>" required>
        <br>
        <label>Спеціальність </label>
            <input type="text" name="special" value="<?=$doc['Спеціальність']?>" required>
        <br>
        <label>Кабінет</label>
            <input type="text" name="cab" value="<?=$doc['Кабінет']?>" required>
        <br>
        <label>Стаж роботи</label>
            <input type="text" name="exp" value="<?=$doc['Стаж']?>" required>
        <br>
